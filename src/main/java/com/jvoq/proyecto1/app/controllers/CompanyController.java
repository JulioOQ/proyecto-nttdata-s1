...
public class CompanyController {
	...
	public Mono<ResponseEntity<Company>> verDetalle(@PathVariable String id){
		return companyService.findById(id).map(c -> ResponseEntity.ok()
				.contentType(MediaType.APPLICATION_JSON)
	...
	public Mono<ResponseEntity<Company>> crear(@RequestBody Company company){
		return companyService.save(company).map(c -> ResponseEntity
				.created(URI.create("/companies".concat(c.getIdEmpresa())))
				.contentType(MediaType.APPLICATION_JSON)
				.body(c));
	}
